...
def test_index_create_with_index_params(session_admin_client, test_case, random_name):
    ...
    for result in results:
        if result['id']['name'] == random_name:
            ...
            if 'max_mem_queue_size' in result.get('hnsw_params', {}):
                assert result['hnsw_params']['max_mem_queue_size'] == test_case.index_params.max_mem_queue_size
            ...
            if 'caching_params' in result.get('hnsw_params', {}):

                assert int(result['hnsw_params']['caching_params']['max_entries']) == test_case.index_params.caching_params.max_entries
                assert int(result['hnsw_params']['caching_params']['expiry']) == test_case.index_params.caching_params.expiry

            if 'merge_params' in result.get('hnsw_params', {}):
                assert result['hnsw_params']['merge_params']['parallelism'] == test_case.index_params.merge_params.parallelism

            if 'healer_params' in result.get('hnsw_params', {}):
                assert int(result['hnsw_params']['healer_params']['max_scan_rate_per_node']) == test_case.index_params.healer_params.max_scan_rate_per_node
                assert int(result['hnsw_params']['healer_params']['max_scan_page_size']) == test_case.index_params.healer_params.max_scan_page_size
                assert int(result['hnsw_params']['healer_params']['re_index_percent']) == test_case.index_params.healer_params.re_index_percent
                assert int(result['hnsw_params']['healer_params']['schedule_delay']) == test_case.index_params.healer_params.schedule_delay
                assert result['hnsw_params']['healer_params']['parallelism'] == test_case.index_params.healer_params.parallelism
